...
class LogiqueDuJeu:
    def __init__(self):
        self.db = Database()
        self.id_partie = self.db.getLastGame()
        self.__functions_dict = {"attaquer": self.attaquer,
                                 "bataille_des": self.bataille_des,
                                 "deplacer_troupes": self.deplacer_troupes,
                                 "changer_tour": self.changer_tour,
                                 "tour": self.tour,
                                 "donner_troupes": self.donner_troupes,
                                 "debut_partie": self.debut_partie,
                                 "placement_troupes": self.placement_troupes,
                                 "change_couleur": self.change_couleur,
                                 "return_plateau": self.return_plateau,
                                 "deroulement": self.deroulement}

    def attaquer(self, database, id_attaquant, id_case_dep, id_case_cib, nb_troupe,
                 nb_troupes_envoyées):  # Possible que si le nb de troupe est strictement supérieur à 1
        ...
        if nb_troupe <= nb_pions_case_dep and id_joueur != id_joueur_ennemie and graphe.verifier_voisins(id_case_dep,
                                                                                                         id_case_cib):
            pertes = self.bataille_des(nb_troupe, nb_pions_case_cib)
            new_nb_troupe_att = nb_troupe - pertes[0]
            new_nb_troupe_def = self.nb_troupe_def - pertes[1]
            ...
            if new_nb_troupe_def < 1:  # Le défenseur a perdu son pays
                ...
                self.change_couleur(idpartie, id_case_cib, id_attaquant)
                self.deplacer_troupes(idpartie, id_case_dep, id_case_cib, nb_troupes_envoyées)
    ...
    def deplacer_troupes(self, case_depart, case_arrivée, nb_troupes):
        ...
        Partie = self.db.getPartie(
            id_partie)  # liste de tuples [(id_case, id_joueur, nb_pions)], avec un tuple par case
        graphe = self.db.graphe
        ...
        if get_joueur(case_depart) != self.tour(id_partie):
            raise CaseNonValide
        joueur = self.tour(id_partie)
        ...
        while not (f.est_vide()):
            a = f.defiler()
            if a not in visited and get_joueur(
                    a) == joueur:  # on verifie que la case nest pas visite ET elle appartient au joueur dont cest le tour
                visited[a] = True
                if a == case_depart:
                    test = True
    ...
    def changer_tour(self):
        ...
        joueurs = self.db.recupere_bdd("joueurs_partie", "id_joueur",
                                       {"id_partie": ("=", id_partie)})  # order by id_joueur limit 1
    ...
    def donner_troupes(self, joueur):
        ...
        nb_territoires = 0
        nb_territoires = len(self.db.recupere_bdd("etat_partie", "id_cases",
                                                  {"id_partie": ("=", self.id_partie), "id_joueur": ("=", id_joueur)}))
    ...
    def debut_partie(self):
        ...
        for id_player in L:
            ...
            self.db.insert_bdd("etat_partie",
                               {"id_partie": id_partie, "id_cases": terres[i], "id_joueur": id_player, "nb_pion": 1})
            i += 1
    ...
    def return_plateau(self):
        dico = dict()
        cases = self.db.getPartie(
            idpartie)  # # Renvoie liste de tuples [(id_case, id_joueur, nb_pions)], avec un tuple par case
        for case in cases:
